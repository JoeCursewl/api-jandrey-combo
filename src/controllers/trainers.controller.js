...
export const getTrainersSaved = async (req, res) => {
  const token = req.headers.authorization;
  if (!token) {
    return res.status(401).json({ message: "No se proporcionó un token" });
  }
  ...
  try {
    ...
    const { id } = req.params;
    const sQuery = `SELECT ts.id, ts.user_id, u.name, u.last_name, ts.trainer_id, t.name_trainer, t.packages_trainer, t.schedule_trainer, t.info_trainer, t.status_trainer, t.created_at, t._id_trainer, ts.saved_at
    ...
    return res.status(200).json({ trainers: saved.rows });
  } catch (error) {
    console.log(error.message);
    return res.status(500).json({ message: error.message });
  }
